...
struct Cli {
    ...
    #[arg(long, action = ArgAction::SetTrue)]
    json: bool,
    /// Skip ICMP ping discovery
    #[arg(long, action = ArgAction::SetTrue)]
    no_ping: bool,

    /// Only run ping discovery and print alive hosts
    #[arg(long, action = ArgAction::SetTrue)]
    ping_only: bool,
...
async fn icmp_ping_addr(ip: IpAddr, timeout_ms: u64) -> bool {
    // Convert IpAddr to string once
    let ip_s = ip.to_string();

    #[cfg(target_os = "windows")]
    let args = ["-n", "1", "-w", &timeout_ms.to_string(), &ip_s];

    #[cfg(not(target_os = "windows"))]
    let args = ["-c", "1", "-W", &((timeout_ms + 999) / 1000).to_string(), &ip_s];
...
async fn main() -> Result<()> {
    ...
    let sem = Arc::new(Semaphore::new(100)); // tune concurrency
        ...
        tasks.push(tokio::spawn(async move {
            match ip_clone.parse::<IpAddr>() {
                Ok(ip) => {
                    ...
                    let alive = icmp_ping_addr(ip, 1000).await;
                    (ip_clone, Some(alive))
                }
                Err(_) => (ip_clone, None),
            }
    ...
    while let Some(Ok((ip_str, maybe_alive))) = tasks.next().await {
        match maybe_alive {
            Some(true) => alive_hosts.push(ip_str),
            Some(false) => (),
            None => alive_hosts.push(ip_str),
        }
    }


// Then iterate over alive_hosts instead of targets:
