...
impl Probe for Pop3Probe {
            ...
            if let Some(reply) = read_chunk(&mut stream).await {
                ...
                if reply.starts_with("+OK") {
                    if let Ok(tls_stream) = upgrade_to_tls(stream, ip).await {
                        return fingerprint_pop3_tls_with_evidence(ip, port, tls_stream, evidence).await;
                    }
                }
            }
        }
    ...
    fn ports(&self) -> Vec<u16> { vec![110, 995] }
...
async fn fingerprint_pop3_tls(ip: &str, port: u16, mut tls_stream: SslStream<TcpStream>) -> Option<ServiceFingerprint> {
