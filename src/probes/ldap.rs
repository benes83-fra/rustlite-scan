...
use crate::probes::helper::{connect_with_timeout, send_and_read, ldap_result_text,upgrade_to_tls};
    ...
    async fn probe(&self, ip: &str, port: u16, timeout_ms: u64) -> Option<ServiceFingerprint> {
        ...
        if let Some( tmp) = {
            ...
            match tokio::time::timeout(Duration::from_millis(300), stream.read(&mut buf)).await {
                Ok(Ok(n)) if n > 0 => { buf.truncate(n); Some(buf) }
                _ => None,
            }
        } {
        ...
        if let Some(resp) = send_and_read(&mut stream, &bind_request, 500, 1000).await {
            ...
            if let Some(pos) = resp.iter().position(|&b| b == 0x61) {
                ...
                evidence.push_str(&format!("LDAP_bind_response: result_code={} ({})", code, text));
            } else {
                if !evidence.is_empty() { evidence.push('\n'); }
                evidence.push_str(&format!("LDAP_bind_raw: {:02x?}", &resp[..std::cmp::min(64, resp.len())]));
            }
        } else {
        ...
        if let Some(rootdse_resp_bytes) = rootdse_resp_bytes_opt.clone() {
            ...
            follow_altservers_and_append(&rootdse_resp_bytes, &attrs, &mut evidence, 3, timeout_ms).await;
        } else {
            if !evidence.is_empty() { evidence.push('\n'); }
            ...
            if resp.iter().any(|&b| b == 0x78) {
                ...
                match upgrade_to_tls(stream, ip).await {
                    Ok(tls_stream) => {
                        ...
                        if let Some(fp) = fingerprint_tls(ip, port, "ldap", evidence.clone(), tls_stream).await {
...
pub fn extract_altservers(resp: &[u8]) -> Vec<String> {
    ...
    if let Some(mut pos) = resp.windows(needle.len()).position(|w| w == needle) {
        ...
        while pos < resp.len() {
            if resp.windows(needle.len()).position(|w| w == needle).is_none() {
                break;
            }
    ...
    for url in urls {
        ...
        match tokio::time::timeout(Duration::from_millis(timeout_ms), TcpStream::connect(&addr)).await {
            Ok(Ok(mut stream)) => {
                ...
                // send and read
                if tokio::time::timeout(Duration::from_millis(500), stream.write_all(&req)).await.is_ok() {
                    let mut buf = vec![0u8; 8192];
                    if let Ok(Ok(n)) = tokio::time::timeout(Duration::from_millis(1000), stream.read(&mut buf)).await {
                        if n > 0 {
                            buf.truncate(n);
                            let decoded = decode_rootdse_response(&buf, attrs);
                            if !evidence.is_empty() { evidence.push('\n'); }
                            evidence.push_str(&format!("altServer {} RootDSE:", url));
                            if !decoded.is_empty() { evidence.push('\n'); evidence.push_str(&decoded); }
                            followed += 1;
                        } else {
                            if !evidence.is_empty() { evidence.push('\n'); }
